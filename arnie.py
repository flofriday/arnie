...
def cmd_bench(args: argparse.Namespace) -> None:
    ...
    reuse_original = args.reuse_original and "original" in builds
    active_builds = [b for b in builds if not (reuse_original and b == "original")]

    repo_keys = {BUILD_CONFIGS[b]["repo_key"] for b in active_builds}
    repos = require_repos(repo_keys)

    timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S")
    run_dir = DATA_DIR / timestamp

    # Resolve reuse source before DATA_DIR is wiped
    reuse_src: Path | None = None
    if reuse_original:
        latest = DATA_DIR / "latest"
        if not latest.exists():
            print(
                "Error: --reuse-original requires existing data in data/latest.",
                file=sys.stderr,
            )
            sys.exit(1)
        prev = (
            latest.parent / latest.readlink()
            if latest.is_symlink()
            else latest.resolve()
        )
        src = prev / "original"
        if not src.exists():
            print(f"Error: no original data found in {prev}.", file=sys.stderr)
            sys.exit(1)
        reuse_src = src
    ...
    for key, commit in commits.items():
        print(f"{key}: {commit}")
    if reuse_src:
        print(f"original: reusing data from {reuse_src.parent.name}")
    ...
    if reuse_src:
        with tempfile.TemporaryDirectory() as _tmp:
            shutil.copytree(reuse_src, Path(_tmp) / "original")
            for d in (DATA_DIR, Path("plots")):
                if d.exists():
                    shutil.rmtree(d)
            run_dir.mkdir(parents=True)
            shutil.copytree(Path(_tmp) / "original", run_dir / "original")
    else:
        for d in (DATA_DIR, Path("plots")):
            if d.exists():
                shutil.rmtree(d)
...
def load_all_runs_total(spec_dir: Path, family: str = "openvadl") -> list[float]:
    """Return total allowed ms per run in order: warmup files then measured files."""
    rename = PASS_RENAME.get(family, {})
    totals = []
    warmups = sorted(
        spec_dir.glob("warmup_*.csv"), key=lambda p: int(p.stem.split("_")[1])
    )
    measured = sorted(
        spec_dir.glob("run_*.csv"), key=lambda p: int(p.stem.split("_")[1])
    )
    for f in warmups + measured:
        with f.open(newline="") as fh:
            reader = csv.DictReader(fh)
            run: dict[str, float] = {}
            for row in reader:
                name = rename.get(row["pass"], row["pass"])
                run[name] = run.get(name, 0.0) + float(row["duration_ms"])
        totals.append(_allowed_total(run))
    return totals
...
# pgfplots fill colour + hatch pattern per bar series index
PGF_STYLES = [
...
# pgfplots color + mark per line series index
_LINE_STYLES = [
    ("blue", "*"),
    ("red", "square*"),
    ("green!60!black", "triangle*"),
    ("orange!80!black", "diamond*"),
    ("purple", "pentagon*"),
    ("teal", "o"),
]
...
def gen_warmup_plot_tex(data_dir: Path, meta: dict, plots_dir: Path) -> Path | None:
    spec = "miniARMv7"
    builds = _builds_ordered({b: None for b in meta.get("builds", [])})
    warmup_n = meta.get("warmup", 0)

    series: dict[str, list[float]] = {}
    for build in builds:
        build_dir = data_dir / build / spec
        if not build_dir.exists():
            continue
        family = BUILD_CONFIGS.get(build, {}).get("family", "openvadl")
        totals = load_all_runs_total(build_dir, family)
        if totals:
            series[build] = totals
    ...
    addplots = []
    for i, (build, totals) in enumerate(series.items()):
        color, mark = _LINE_STYLES[i % len(_LINE_STYLES)]
        coords = " ".join(f"({j + 1},{t:.2f})" for j, t in enumerate(totals))
        addplots.append(
            f"\\addplot[color={color}, mark={mark}] coordinates {{{coords}}};\n"
            f"\\addlegendentry{{{build}}}"
        )
    ...
    body = (
        "\n".join(addplots)
        + f"\n\\draw[dashed, gray!60, thick] (axis cs:{sep},0) -- (axis cs:{sep},100000);"
    )
    ...
    tex = (
        ...
        + "\\end{tikzpicture}%\n"
        + "}\n"
        + "\\caption{Compile time per run for \\texttt{miniARMv7} including warmup (left of dashed line).}\n"
        + "\\label{fig:warmup_miniARMv7}\n"
        + "\\end{figure}\n"
    )
    out = plots_dir / "warmup_miniARMv7.tex"
    out.write_text(tex)
    return out
...
def cmd_plot(args: argparse.Namespace) -> None:
    ...
    warmup_plot = gen_warmup_plot_tex(data_dir, meta, plots_dir)
    if warmup_plot:
        snippets.append(warmup_plot)
    for s in snippets:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bench.add_argument(
        "--reuse-original",
        action="store_true",
        help="copy original build data from data/latest instead of re-running it",
    )
